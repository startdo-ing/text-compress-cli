...
describe("cli path auto-detection", () => {
	...
	it("treats a missing compress path as inline text", () => {
		...
		expect(readFileSync(join(dir, "inline.de.txt"), "utf-8")).toBe(
			"inline payload",
		);
	});
	...
	it("auto-splits large compress output at 30,000 characters", () => {
		...
		expect(
			readFileSync(join(dir, "large-out.1.txt"), "utf-8").length,
		).toBeLessThanOrEqual(30_000);
		expect(
			readFileSync(join(dir, "large-out.2.txt"), "utf-8").length,
		).toBeGreaterThan(0);
		runCli([
			"decompress",
			join(dir, "large-out.1.txt"),
			"-o",
			join(dir, "large-restored.txt"),
		]);
		expect(readFileSync(join(dir, "large-restored.txt"), "utf-8")).toBe(
			payload,
		);
	});
});
