...
import { brotliCompressSync, constants as zlibConstants, createBrotliCompress } from "node:zlib";
import {
...
function estimatedEncodedLength(binaryBytes: number, encoding: Encoding): number {
...
function encodeBinaryFileToTextFiles(
  ...
  const totalParts = split ? Math.ceil(encodedLength / split) : 1;
  const paths: string[] = [];
  ...
  try {
    if (encoding === 64) {
      ...
      const padded = Buffer.concat([Buffer.from([padLength]), raw, Buffer.alloc(padLength)]);
...
export async function compressFolderToPath(
  ...
  try {
    ...
    const outputPaths = encodeBinaryFileToTextFiles(compressedPath, outputPath, encoding, split);
    return { ...stats, compressedBytes, outputPaths };
  } finally {
